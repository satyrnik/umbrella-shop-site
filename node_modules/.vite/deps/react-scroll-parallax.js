...
function useParallaxController() {
  const parallaxController = (0, import_react.useContext)(ParallaxContext);
...
var ParallaxBanner = (props) => {
  const [targetElement, setTargetElement] = (0, import_react.useState)(null);
  ...
  function renderLayers() {
    if (targetElement) {
      if (layers && layers.length > 0) return layers.map((layer, i) => import_react.default.createElement(ParallaxBannerLayer, {
        ...layer,
        targetElement,
        key: `layer-${i}`,
        testId: `layer-${i}`
  ...
  function renderChildren() {
    if (targetElement) return import_react.default.Children.map(props.children, (child) => {
      const item = child;
      if ((item === null || item === void 0 ? void 0 : item.type) === ParallaxBannerLayer) return import_react.default.cloneElement(item, { targetElement });
      return child;
    });
    return null;
  }
...
function ParallaxProvider(props) {
  ...
  return import_react.default.createElement(ParallaxContext.Provider, { value: controller.current }, props.children);
}
export {
  EasingPreset,
  Parallax,
